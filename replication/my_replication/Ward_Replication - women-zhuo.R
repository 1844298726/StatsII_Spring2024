...
# Date: September, 2018
setwd('E:/TCD/ST2/Pre/Public Attitudes toward Young Immigrant Men')
library(data.table)
library(lmtest)
...
# used in plotting below 用于绘图
clust_confint <- function(Mod, Cov){
  tt <- qt(c(0.025, 0.975), Mod$df.residual)#计算t分位数
  se <- sqrt(diag(Cov))
...
# 删除不符合目标年龄范围（18-75岁）或在州、性别或教育程度方面缺失值的36名受访者。
# 在计算权重之前，删除这些数据。
# 此数据用于所有加权估计。
...
# Table A.2
resp_level <- unique(data[ , .(id, age, as.numeric(gender == "Female"), duration_seconds, edu2)]) #respondent-level data for summary stats (all respondents)
...
# Table A.3 (Age)
resp_levelNW <- unique(datw[ , .(id, age, gender, edu2, state, ps_weights, duration_seconds)]) #respondent-level data (only respondents in estimation sample)
# 创建分类年龄变量
...
# Table A.3 (gender)
resp_levelNW[ , male := gender == "Male"]
round(resp_levelNW[ , mean(male)],4)*100 # Unweighted
round(resp_levelNW[ , weighted.mean(male, ps_weights)],4)*100 # Weighted
...
# Model for Figure 1 and Table B.1, Model 1.模型用于图1和表B.1，分子1。
# 使用factor(group_edu)、men0、men25、men75、men100作为自变量拟合线性模型
main <- lm(settle_binary ~ factor(group_edu) + men0 + men25 + men75 + men100, weights = ps_weights, data = datw)
# 计算主要模型的标准误差的矫正系数
main_vcov <- cluster.vcov(main, datw[ , "id"])
# 输出主要模型的系数估计值
(main_coefs <- coeftest(main, main_vcov))

# Footnote 14, main text脚注14，主文
# 对主要模型进行线性假设检验
linearHypothesis(main, "men25 - men0 = 0", vcov = main_vcov)

# Table B.1, Model 2表B.1，模型2
# 使用factor(group_edu)、men0、men25、men75、men100作为自变量拟合线性模型
...
# Figure 1 (Save as 5in x 7in to recreate paper)图1（保存为5英寸x 7英寸以重现论文）
# 绘制图形所需的数据
...
linearHypothesis(sec_mod, "men0 - men25 = 0", vcov = sec_vcov)# 检验 men0 和 men25 之间的差异
linearHypothesis(sec_mod, "men100 - men75 = 0", vcov = sec_vcov)# 检验 men100 和 men75 之间的差异

# Table B.2, Model 4
sec_modNW <- lm(security_binary ~ factor(group_edu) + men0 + men25 + men75 + men100, data = data)
sec_vcovNW <- cluster.vcov(sec_modNW, data[ , id])
(sec_coefNW <- coeftest(sec_modNW, sec_vcovNW))# 输出系数估计结果

# Figure 2, Panel 3, and Table B.2, Model 5
# 图 2，面板 3，以及表 B.2，模型 5
...
(culture_coef <- coeftest(culture_mod, culture_vcov)) # 输出系数估计结果
# 测试文本中提到的效应差异
# test differences between effects as mentioned in text
linearHypothesis(culture_mod, "men0 - men25", vcov = culture_vcov) # 检验 men0 和 men25 之间的差异
linearHypothesis(culture_mod, "men25 - men75", vcov = culture_vcov)# 检验 men25 和 men75 之间的差异
linearHypothesis(culture_mod, "men75 - men100", vcov = culture_vcov)# 检验 men75 和 men100 之间的差异
...
(culture_coefNW <- coeftest(culture_modNW, culture_vcovNW))# 输出系数估计结果
...
texreg(list(
  ...
  culture_modNW),
  override.se = list(
    job_coef[,2],# 用 job_coef 的标准误覆盖默认的标准误
    job_coefNW[,2],
    sec_coef[,2],
    sec_coefNW[,2],
    culture_coef[,2],
    culture_coefNW[,2]),
  override.pvalues = list(
    job_coef[,4],# 用 job_coef 的 p 值覆盖默认的 p 值
    job_coefNW[,4],
    sec_coef[,4],
    sec_coefNW[,4],
    culture_coef[,4],
    culture_coefNW[,4]),
  custom.coef.map = list(
    '(Intercept)' = "Intercept", # 将 '(Intercept)' 映射为 '截距'
    'men0' = "0%",
    'men25' = "25%",
    'men75' = "75%",
    'men100' = "100%",
    'factor(group_edu)10' = "10%",# 将 'factor(group_edu)10' 映射为 '10%'
    'factor(group_edu)20' = "20%",
    'factor(group_edu)30' = "30%"
  ),
  booktabs = TRUE,       # 使用 booktabs 格式
  dcolumn = TRUE,        # 使用 dcolumn 格式
  digits = 3             # 数字保留三位小数
)
...
male_vcov <- cluster.vcov(Female_mod, datw[gender == "Male", id])
...
# Table D.1, Model 1
datw[ , lmc_threat := age <= 30 & gender == "Female" & edu2 %in% c("Low")]
lmc_threatA_interact <- lm(settle_binary ~  factor(group_edu) + men0*lmc_threat + men25*lmc_threat + men75*lmc_threat + men100*lmc_threat, weights = ps_weights, data = datw[])
...
# Table D.1, Model 2
datw[ , lmc_threat := age <= 35 & gender == "Female" & edu2 %in% c("Low")]
...
# Table D.1, Model 3
datw[ , lmc_threat := age <= 40 & gender == "Female" & edu2 %in% c("Low")]
...
# Table D.1, Model 4
datw[ , lmc_threat := age <= 30 & gender == "Female" & edu2 %in% c("Low", "Medium")]
...
# Table D.1, Model 5
datw[ , lmc_threat := age <= 35 & gender == "Female" & edu2 %in% c("Low", "Medium")]
...
# Table D.1, Model 6
datw[ , lmc_threat := age <= 40 & gender == "Female" & edu2 %in% c("Low", "Medium")]
...
# 脚注1，附录A（对3个双重面试受访者进行替代处理的稳健性检验）
# 创建指示器，用于标识三个受访者是否存在双重面试
# 将“datw”更改为“data”，以在未加权数据上进行这些稳健性测试。
...
# .N 是每个受访者完成的选择任务数。
# 一次完整的调查是8个任务，两次完整的是16个任务。
# 稳健性检验1：重新拟合主模型，排除这三个受访者
# ND代表“无双重面试”
...
# 稳健性检验2：重新拟合主模型，为这三个个体的每次面试分别设定独立的聚类。
# ROBUSTNESS 2: refit main model with separate clusters for each of these three individuals' interviews. 
datw[ , id2 := paste0(id,duration_seconds)] # use interview length to differntiate the double interviews.# 使用面试时长来区分双重面试。
# SC代表“独立聚类”
...
# （受访者ID 767同时开始了两次面试。
# 我将他的持续时间较短的面试视为他的第一次面试，
# 但这与将另一次面试视为第一次面试是一致的。）
# 创建第一次面试的指示器，
# 然后将这个指示器对第二次面试设置为假
# （根据上面显示的代码信息）
...
# 代码生成这些稳健性检验和主要结果的Tex表。
# 注意，由于排除了双重面试的受访者/第二次面试，观测数量的变化。
